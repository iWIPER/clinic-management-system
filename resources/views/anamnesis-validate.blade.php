...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sig-img-wrap { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; padding: 10px; text-align: center; }
        .sig-img-wrap img { max-height: 56px; max-width: 260px; }
        .sig-meta-row { display: flex; justify-content: space-between; font-size: 11px; color: #64748b; margin-top: 8px; }
        .badge-signed { background: #f0fdf4; color: #15803d; border: 1px solid #bbf7d0; }
        .badge-unsigned { background: #fffbeb; color: #b45309; border: 1px solid #fde68a; }
    </style>
</head>
<body>
    <div class="card">
        @if($valid)
            ...
            @if(isset($signature_info))
            <div class="sig-section">
                <div class="sig-section-label">Assinatura Eletrônica</div>
                <span class="badge {{ $signature_info['signed'] ? 'badge-signed' : 'badge-unsigned' }}" style="margin-bottom:10px;">
                    {{ $signature_info['signed'] ? '✓ Assinada eletronicamente' : '⌛ Aguardando assinatura' }}
                </span>
                @if($signature_info['signed'])
                <table class="table" style="margin-top:8px;">
                    <tr>
                        <td>Signatário</td>
                        <td>{{ $signature_info['patient_name'] }}</td>
                    </tr>
                    @if($signature_info['signed_at'])
                    <tr>
                        <td>Data da assinatura</td>
                        <td>{{ $signature_info['signed_at'] }}</td>
                    </tr>
                    @endif
                    <tr>
                        <td>Método</td>
                        <td>{{ $signature_info['method'] }}</td>
                    </tr>
                </table>
                @endif
            </div>
